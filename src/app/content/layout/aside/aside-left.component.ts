...
export class AsideLeftComponent implements OnInit, AfterViewInit {
	...
	outsideTm: any;
	sites: any = [];
	public menuAsideMinimized: boolean = false;
	...
	ngOnInit() {
		...
		this.menuAsideService.getLeftAsideMenuMinimized().subscribe(value => {
			this.menuAsideMinimized = value;
		...
		this.statisticsService.getTotalStatistics().subscribe((data:any) => {
			...
			if (this.sites) {
				const menuConfig = this.menuConfigService.getModel();
				const sitesMenu = [];
				...
				this.sites.forEach((item) => {
					...
					sitesMenu.push(siteItem)
				});

				let mysites = _.findWhere(menuConfig.config.aside.items, {title: 'My sites'});
				if (!mysites) {
					mysites = {
						title: 'My sites',
						root: true,
				...
				mysites.submenu = sitesMenu;
				this.menuConfigService.setModel(menuConfig);
			}
		} );
	}
